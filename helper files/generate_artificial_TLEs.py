...
def generate_tle(name, inclination, semi_major_axis, norad_id):
    ...
    raan = random.uniform(0, 360)  # Random RAAN in degrees
    raan_str = f"{raan:8.4f}"  # Format RAAN to 4 decimal places

    arg_of_perigee = random.uniform(0, 360)  # Random argument of perigee in degrees
    arg_of_perigee_str = f"{arg_of_perigee:8.4f}"  # Format argument of perigee to 4 decimal places

    mean_anomaly = random.uniform(0, 360)  # Random mean anomaly in degrees
    mean_anomaly_str = f"{mean_anomaly:8.4f}"  # Format mean anomaly to 4 decimal places
    ...
    tle_line_2 = f"2 {norad_id:05d} {inclination_str} {raan_str} 0  {arg_of_perigee_str} {mean_anomaly_str} {mean_motion_str}"
...
generate_and_write_tles("generated_tles_random.txt", inclination_range, semi_major_axis_range)
