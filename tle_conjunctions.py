...
def main():
    ...
    for i in range(int(len(tle_lines)/3)):
        ...
        # for each object, determine the range in orbital range based on semimajor axis and eccentricity (this will be used to filter the objects we search for conjunctions with)
    ...
    # Assuming compute_conjunction_distance is defined elsewhere
    ...
    try:
        with open('data/conj_params_' + str(n) + '.pkl', 'rb') as f:
            dist_a, dist_b, alt_a, alt_b, T_syn = pickle.load(f)
        ...
        T_syn = np.full((n, len(tle_l1)), np.nan)
        t1 = time.time()
        ...
        with ProcessPoolExecutor(max_workers=os.cpu_count()) as executor:
            ...
            for idx, future in enumerate(futures):
                dist_a[idx, :], dist_b[idx, :], alt_a[idx, :], alt_b[idx, :], T_syn[idx, :] = future.result()
                print(idx / n)
        ...
        try:
            with open('data/conj_params_' + str(n) + '.pkl', 'wb') as f:
                pickle.dump((dist_a, dist_b, alt_a, alt_b, T_syn), f)
    ...
    T_syn_valid = T_syn.flatten()[valid_indices_a] #use a or b?
    ...
    # plt.figure()
    # plt.scatter(alt_a_valid, dist_a_valid, color='tab:blue', label='a')
    # plt.scatter(alt_b_valid, dist_b_valid, color='tab:orange', label='b')
    # plt.xlabel('Conjunction altitude (km)')
    # plt.ylabel('Closest approach distance (km)')
    # plt.legend()
    # plt.show()

    # plot a 2D histogram of alt_a vs dist_a
    # create bins from min(alt_valid) to max(alt_valid) and min(dist_valid) to max(dist_valid)
    # bins = [np.arange(np.nanmin(alt_valid), np.nanmax(alt_valid), 1), np.arange(np.nanmin(dist_valid), np.nanmax(dist_valid), 1)]
    # # count how many points are in each bin
    # hist, xedges, yedges = np.histogram2d(alt_valid, dist_valid, bins=bins)
    # # plot the histogram
    # plt.figure()
    # plt.imshow(hist, cmap='viridis', interpolation='none', aspect = 'auto', extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]])
    # plt.colorbar()
    # plt.xlabel('Conjunction altitude (km)')
    # plt.ylabel('Closest approach distance (km)')
    # plt.show()
    ...
    '''
    plt.figure()
    plt.hist2d(alt_valid, dist_valid, bins=(100,100), cmap='viridis', norm=plt.matplotlib.colors.LogNorm())
    plt.xlabel('Conjunction altitude (km)')
    plt.ylabel('Closest approach distance (km)')
    plt.colorbar()
    plt.show()

    # count the number of close approaches (dist < 1 km) for within each conjunction altitude bin
    # make bins for the conjunction altitude
    alt_bins = np.arange(np.nanmin(alt_valid), np.nanmax(alt_valid), 1)
    # initialize the number of close approaches for each bin
    close_approaches = np.zeros(len(alt_bins)-1)
    # loop through each bin
    for i in range(len(alt_bins)-1):
        # find the indices of alt_valid that fall within the bin
        indices = (alt_valid >= alt_bins[i]) & (alt_valid < alt_bins[i+1])
        # count the number of close approaches in the bin
        close_approaches[i] = np.sum(dist_valid[indices] < 1)
    # plot the number of close approaches in each bin
    plt.figure()
    plt.bar(alt_bins[:-1], close_approaches, width=1)
    # make it log scale
    plt.yscale('log')
    plt.xlabel('Conjunction altitude (km)')
    ...
    # for each element in dist_min, take the minimum value of dist_a or dist_b. If that element is nan for dist_a or dist_b, take the other value. If both are nan, then make it nan
    dist_min = np.minimum(dist_a, dist_b)

    # make each element of dist_min where i < j, take the value of dist_min[j,i]
    for i in range(len(tle_l1)):
        for j in range(i):
            dist_min[i,j] = dist_min[j,i]
    
    # plot dist_a as a heatmap
    plt.figure()
    # make log-scaled colorbar
    plt.imshow(dist_min, cmap='jet', interpolation='none', aspect = 'auto', norm = plt.matplotlib.colors.LogNorm())
    # make the x and y labels the object names (but make them very small - the same size as their pixel width)
    # plt.xticks(np.arange(len(obj_name)), obj_name)
    # # make the xticks vertical
    # plt.xticks(rotation=90)
    # plt.yticks(np.arange(len(obj_name)), obj_name)
    plt.colorbar()
    plt.xlabel('Object j')
    plt.ylabel('Object i')
    plt.title('Closest approach distance between objects a')
    plt.show()
    ...
    below_threshold_indices = np.where((dist_a < threshold) & (T_syn < 7776000))
    below_threshold_indices_b = np.where((dist_b < threshold) & (T_syn < 7776000))
    ...
    def time_until_collision(tle1_line1, tle1_line2, tle2_line1, tle2_line2, a_or_b):
        ...
        #    collision = False
        #    return time_to_collision, intersection_point, T_syn, collision
        ...
        t1a = t1a[t1a <= 7786000]
        t2a = t2a[t2a <= 7786000]
        #print(f"T_syn:, {T_syn}, Length of t1a: {len(t1a)}, Last values: {t1a[-4]}, {t1a[-3]}, {t1a[-2]}, {t1a[-1]}")
        #print(f"T_syn:, {T_syn}, Length of t2a: {len(t2a)}, Last values: {t2a[-4]}, {t2a[-3]}, {t2a[-2]}, {t2a[-1]}")
    ...
    # Write header
        ...
        for i, j in tqdm(below_threshold_pairs, desc="Processing Pairs", unit="pair"):
            ...
            time_to_collision_a, intersection_point, T_syn, collision = time_until_collision(tle1_line1, tle1_line2, tle2_line1, tle2_line2, a_or_b)
            alt = np.linalg.norm(intersection_point) - 6378
            ...
            writer.writerow([name1, norad_id1, name2, norad_id2, intersection_point, a_or_b, alt, collision, time_to_collision_days, T_syn_days, 1/T_syn_days])
    ...
    # scatter plot c_alt vs d for all i and j
    # plt.figure()
    # plt.scatter(d.flatten(), c_alt.flatten())
    # plt.xlabel('Closest approach distance (km)')
    # plt.ylabel('Conjunction altitude (km)')
    # plt.show()


    # plt.figure()
    # # get norad ID for reference object in i
    # # norad_id = tle1_line2.split()[1]
    # # plt.title('Closest approach distance for ' + norad_id)
    # # plt.hist(d[i,:], bins=1000)
    # # repeat above line, but instead of 1000 bins, the bins should be 1 km wide (so however many it takes)
    # plt.hist(d[i,:], bins=np.arange(0, np.nanmax(d[i,:]), 1))
    # plt.xlabel('Closest approach distance (km)')
    ...
    # tle2_line1 = tle_l1[1]
    # tle2_line2 = tle_l2[1]
    # find_orbital_intersection(tle1_line1, tle1_line2, tle2_line1, tle2_line2)
...
def find_min_conj_dist(tle1_line1, tle1_line2, tle2_line1, tle2_line2):
    ...
    return dist_a, dist_b, alt_a, alt_b, T_syn
...
def compute_conjunction_distance(i, tle_l1, tle_l2, a_range):
    ...
    T_syn = np.full(len(tle_l1), np.nan)
    ...
    for j in range(len(tle_l1)):
        ...
        dist_a[j], dist_b[j], alt_a[j], alt_b[j], T_syn[j] = find_min_conj_dist(tle1_line1, tle1_line2, tle2_line1, tle2_line2)
    print(i)
    return dist_a, dist_b, alt_a, alt_b, T_syn
